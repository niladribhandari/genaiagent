...
class DeveloperAssistantServer {
  public server: Server; // Make server public
  private openai: any;
  private activeWorkflows: Map<string, Workflow> = new Map();
  private pendingApprovals: Map<string, ApprovalRequest> = new Map();
  private orchestrator: AgentOrchestrator;
  private workflowOrchestrator: WorkflowOrchestrationAgent;
    ...
    switch (name) {
      case "analyze_project_structure":
        return await this.analyzeProjectStructure(
          (args as any).projectPath as string, 
          ((args as any).maxDepth as number) || 3
        );

      case "generate_code_template":
        return await this.generateCodeTemplate(
          (args as any).templateType as string, 
      ...
      case "analyze_code_quality":
        return await this.analyzeCodeQuality(
          (args as any).filePath as string, 
          (args as any).language as string | undefined
        );

      case "generate_documentation":
        return await this.generateDocumentation(
          (args as any).filePath as string, 
          (args as any).docType as string
        );

      case "git_workflow_helper":
        return await this.gitWorkflowHelper(
          (args as any).action as string, 
          (args as any).repositoryPath as string, 
          (args as any).files as string[] | undefined
        );

      case "summarize_project_functionality":
        return await this.summarizeProjectFunctionality(
          (args as any).projectPath as string,
          (args as any).language as string
        );

      // Enhanced Agentic Workflow Tools
      case "start_enhanced_workflow":
        return await this.startEnhancedWorkflow(
          (args as any).requirements as string,
          (args as any).technology as string,
          (args as any).outputPath as string,
          {
            templateId: (args as any).templateId as string,
            approvalMode: (args as any).approvalMode as string || "interactive",
            config: (args as any).config as any || {}
          }
        );

      case "get_enhanced_workflow_status":
        return await this.getEnhancedWorkflowStatus(
          (args as any).workflowId as string
        );

      case "handle_enhanced_approval":
        return await this.handleEnhancedApproval(
          (args as any).workflowId as string,
          (args as any).phaseId as string,
          (args as any).action as string,
          {
            modifications: (args as any).modifications,
            feedback: (args as any).feedback as string,
            userId: (args as any).userId as string
          }
        );
      ...
      case "get_workflow_templates":
        return await this.getWorkflowTemplates(
          (args as any).technology as string
        );

      case "start_development_workflow":
        return await this.startDevelopmentWorkflow(
          (args as any).requirements as string,
          (args as any).technology as string,
          (args as any).outputPath as string,
          (args as any).approvalMode as string || "interactive",
          (args as any).workflowConfig as any || {}
        );
      ...
      case "approve_workflow_phase":
        return await this.approveWorkflowPhase(
          (args as any).workflowId as string,
          (args as any).phaseId as string,
          (args as any).action as string,
          (args as any).modifications as any,
          (args as any).feedback as string
        );
      ...
      case "compile_project_with_agent":
        const compileAgent = this.orchestrator.getAgent('compilation') as any;
        const compileResult = await compileAgent.compileProject(
          args.projectPath as string,
          args.language as string,
          args.buildTool as string
        );
        return {
          content: [
            {
      ...
      case "generate_code_with_agent":
        const genAgent = this.orchestrator.getAgent('generation') as any;
        const codeGenResult = await genAgent.generateProject(
      ...
      case "review_code_with_agent":
        const reviewAgent = this.orchestrator.getAgent('review') as any;
        const reviewResult = await reviewAgent.reviewProject(
          args.filePath as string,
          args.criteria as string[] || ['quality', 'security']
        );
        return {
          content: [
            {
      ...
      case "search_github_with_agent":
        const githubAgent = this.orchestrator.getAgent('github') as any;
        const githubResult = await githubAgent.searchCodebase(
      ...
      case "search_web_with_agent":
        const webAgent = this.orchestrator.getAgent('web') as any;
        const webResult = await webAgent.searchWeb(
          args.query as string,
      ...
      case "generate_api_spec_with_agent":
        ...
        try {
          const apiAgent = this.orchestrator.getAgent('apispec') as any;
          ...
          const apiSpecResult = await apiAgent.generateAPISpec(
            args.requirements as string,
            args.outputPath as string || 'generated-api',
            args.format as string || 'openapi'
          );
      ...
      case "compile_code_with_agent":
        return await this.compileCodeWithAgent(
          args.projectPath as string,
          args.technology as string
        );

      case "review_code_with_agent":
        return await this.reviewCodeWithAgent(
          args.projectPath as string,
          args.technology as string
        );
  ...
  private async callToolHandler(name: string, args: any) {
  ...
  private async analyzeProjectStructure(projectPath: string, maxDepth: number): Promise<any> {
    try {
  ...
  private async buildProjectStructure(dirPath: string, currentDepth: number, maxDepth: number): Promise<ProjectStructure> {
    ...
    if (currentDepth < maxDepth) {
      try {
        ...
        const filteredEntries = entries.filter(entry => 
          !entry.startsWith('.') || ['.gitignore', '.env.example'].includes(entry)
        );
  ...
  private async getProjectStats(projectPath: string) {
    let fileCount = 0;
    let dirCount = 0;
    ...
    const traverse = async (currentPath: string) => {
      try {
        ...
        for (const entry of entries) {
          if (entry.name.startsWith('.') && !entry.name.includes('git')) continue;
  ...
  private getProjectRecommendations(structure: ProjectStructure, stats: any): string[] {
    ...
    if (!hasReadme) recommendations.push("Consider adding a README.md file for project documentation");
    if (!hasGitignore) recommendations.push("Consider adding a .gitignore file");
  ...
  private findFile(structure: ProjectStructure, fileName: string): boolean {
    if (structure.type === 'file' && structure.name === fileName) return true;
  ...
  private async generateCodeTemplate(templateType: string, name: string, options: any): Promise<any> {
    const templates = {
...
export default ${name};`;
  ...
  private generateExpressRoute(name: string, options: any): string {
...
export default router;`;
  ...
  private generateTypescriptClass(name: string, options: any): string {
...
export const getConfig = () => {
  ...
  private getSuggestedFileName(templateType: string, name: string): string {
    const fileNameMap = {
    ...
    return fileNameMap[templateType as keyof typeof fileNameMap] || `${name}.ts`;
  }

  private async analyzeCodeQuality(filePath: string, language?: string): Promise<any> {
    try {
  ...
  private performCodeAnalysis(code: string, language: string) {
    ...
    lines.forEach(line => {
      ...
      if (!trimmed) analysis.metrics.emptyLines++;
  ...
  private async generateDocumentation(filePath: string, docType: string): Promise<any> {
    try {
      const code = await fs.readFile(filePath, 'utf-8');
  ...
  private createDocumentation(code: string, docType: string, language: string, filePath: string): string {
  ...
  private generateReadme(fileName: string, code: string, language: string): string {
    return `# ${fileName}
...
`;
  ...
  private generateApiDocs(code: string, language: string): string {
  ...
  private generateInlineComments(code: string, language: string): string {
    const lines = code.split('\n');
...
`;
  ...
  private extractFunctionsAndClasses(code: string): string {
    const functions = this.extractFunctions(code);
  ...
  private async gitWorkflowHelper(action: string, repositoryPath: string, files?: string[]): Promise<any> {
    try {
      let result;
  ...
  private getGitStatus(repositoryPath: string) {
    try {
  ...
  private async suggestCommitMessage(repositoryPath: string, files?: string[]): Promise<any> {
    try {
  ...
  private analyzeChangesForCommitMessage(diff: string, files?: string[]) {
    const suggestions = [];
  ...
  private generateConventionalCommits(suggestions: string[]) {
  ...
  private getBranchInfo(repositoryPath: string) {
    try {
  ...
  private getRecentChanges(repositoryPath: string) {
    try {
  ...
  private async summarizeProjectFunctionality(projectPath: string, language: string): Promise<any> {
    try {
      ...
      switch (language) {
        ...
        case ".net":
          try {
            ...
            if (csproj) meta = await fs.readFile(path.join(projectPath, csproj), "utf-8");
          } catch {}
          break;
      }
      try {
      ...
      async function walk(dir: string) {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory() && !entry.name.startsWith('.')) await walk(fullPath);
          else if (entry.isFile() && [".js", ".ts", ".tsx", ".java", ".cs"].some(ext => entry.name.endsWith(ext))) files.push(fullPath);
        }
      }
  ...
  private async startEnhancedWorkflow(
    requirements: string,
    technology: string,
    outputPath: string,
    options: {
    ...
    try {
      const result = await this.workflowOrchestrator.startWorkflow(
        ...
        outputPath,
        {
          approvalMode: options.approvalMode as any || 'interactive',
  ...
  private async getEnhancedWorkflowStatus(workflowId: string): Promise<any> {
    try {
  ...
  private async handleEnhancedApproval(
    workflowId: string,
    phaseId: string,
    action: string,
    options: {
    ...
    try {
      const result = await this.workflowOrchestrator.handleApproval(
        workflowId,
        phaseId,
        action as any,
        options
  ...
  private async getEnhancedPendingApprovals(): Promise<any> {
    try {
  ...
  private async getWorkflowTemplates(technology?: string): Promise<any> {
    try {
  ...
  private estimateCompletion(workflow: EnhancedWorkflow): string {
  ...
  private async startDevelopmentWorkflow(
    requirements: string,
    technology: string,
    outputPath: string,
    approvalMode: string = "interactive",
    workflowConfig: any = {}
  ): Promise<any> {
  ...
  private async approveWorkflowPhase(
    workflowId: string,
    phaseId: string,
    action: string,
    modifications?: any,
  ...
  private async executeWorkflowPhase(workflow: Workflow, phaseId: string): Promise<void> {
    const phase = workflow.phases.find(p => p.id === phaseId);
    if (!phase) return;
    ...
    try {
      ...
      const agentResult: AgentResult = await this.orchestrator.executeWorkflowPhase(phaseId, input);
  ...
  private getPhaseResult(workflow: Workflow, phaseId: string): any {
    const phase = workflow.phases.find(p => p.id === phaseId);
    return phase?.result || null;
  }

  private async proceedToNextPhase(workflow: Workflow): Promise<void> {
  ...
  private async executeSpecificationPhase(workflow: Workflow): Promise<any> {
  ...
  private async executeCodeGenerationPhase(workflow: Workflow): Promise<any> {
  ...
  private async executeCodeReviewPhase(workflow: Workflow): Promise<any> {
  ...
  private async executeCompilationPhase(workflow: Workflow): Promise<any> {
  ...
  private async executeDeploymentPhase(workflow: Workflow): Promise<any> {
    // Simulate deployment
    return {
  ...
  private async listGeneratedProjects(): Promise<any> {
    try {
  ...
  private async compileCodeWithAgent(projectPath: string, technology: string): Promise<any> {
    try {
      const compilationAgent = this.orchestrator.getAgent('compilation');
      
      if (!compilationAgent || typeof (compilationAgent as any).compileProject !== 'function') {
        throw new Error('Compilation agent not available');
      }

      const result = await (compilationAgent as any).compileProject(
        projectPath,
        technology,
        {} // buildOptions
      );
      ...
      const extractFileNameFromMessage = (message: string): string | null => {
        const fileMatch = message.match(/\/([^\/]+\.java):\[/);
        return fileMatch ? fileMatch[1] : null;
      };

      const extractLineFromMessage = (message: string): number | null => {
        const lineMatch = message.match(/:?\[(\d+),\d+\]/);
        return lineMatch ? parseInt(lineMatch[1], 10) : null;
      };

      const extractColumnFromMessage = (message: string): number | null => {
      ...
      if (result.data) {
        ...
        if (compilationData.issues && Array.isArray(compilationData.issues)) {
          ...
          const transformedErrors = compilationData.issues
            .filter((issue: any) => issue.severity === 'error') // Only show errors for now
            .map((issue: any) => ({
          ...
          const transformedErrors = compilationData.errors.map((error: any) => ({
  ...
  private async reviewCodeWithAgent(projectPath: string, technology: string): Promise<any> {
    try {
      const reviewAgent = this.orchestrator.getAgent('review');
      
      if (!reviewAgent || typeof (reviewAgent as any).reviewProject !== 'function') {
        throw new Error('Review agent not available');
      }

      const result = await (reviewAgent as any).reviewProject(
        projectPath,
  ...
  private async analyzeProjectDirectory(projectPath: string): Promise<{ fileCount: number; totalSize: number }> {
    let fileCount = 0;
    let totalSize = 0;

    const analyzeRecursive = async (dirPath: string) => {
  ...
  private detectProjectTechnology(projectPath: string): string {
    try {
...
app.get('/specs/list', async (req: Request, res: Response) => {
...
const getSpecHandler = async (req: any, res: any) => {
