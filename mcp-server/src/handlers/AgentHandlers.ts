...
import { 
  CodeCompilationAgentWrapper,
  CodeReviewAgentWrapper,
  CodeGenerationAgentWrapper,
  APISpecWriterAgentWrapper,
  FixCompilationIssuesAgentWrapper
} from '../agents.js';
...
export class CompileCodeWithAgentHandler extends BaseToolHandler {
  async handle(args: any, context: ServerContext): Promise<ToolResult> {
    ...
    const compileAgent = context.orchestrator.getAgent('compilation') as CodeCompilationAgentWrapper;
    ...
    if (compileResult && compileResult.data && compileResult.data.issues && Array.isArray(compileResult.data.issues)) {
      compileResult.data.issues = compileResult.data.issues.map((issue: any) => {
      ...
      compileResult.data.errors = compileResult.data.errors.map((error: any) => {
...
export class ReviewCodeWithAgentHandler extends BaseToolHandler {
  async handle(args: any, context: ServerContext): Promise<ToolResult> {
    ...
    const reviewAgent = context.orchestrator.getAgent('review') as CodeReviewAgentWrapper;
...
export class GenerateCodeWithAgentHandler extends BaseToolHandler {
  async handle(args: any, context: ServerContext): Promise<ToolResult> {
    ...
    const genAgent = context.orchestrator.getAgent('generation') as CodeGenerationAgentWrapper;
...
export class GenerateApiSpecWithAgentHandler extends BaseToolHandler {
  async handle(args: any, context: ServerContext): Promise<ToolResult> {
    ...
    const apiAgent = context.orchestrator.getAgent('apispec') as APISpecWriterAgentWrapper;
...
export class FixCompilationIssuesWithAgentHandler extends BaseToolHandler {
  async handle(args: any, context: ServerContext): Promise<ToolResult> {
    ...
    const fixAgent = context.orchestrator.getAgent('fixcompilation') as FixCompilationIssuesAgentWrapper;
...
export class EnhancedCompileCodeHandler extends BaseToolHandler {
  ...
  private transformToSimplifiedFormat(originalResult: any, projectPath: string): any {
    /**
    ...
    const transformedIssues = issues.map((issue: any, index: number) => ({
    ...
    const errorCount = transformedIssues.filter((i: any) => i.severity === 'error').length;
    const warningCount = transformedIssues.filter((i: any) => i.severity === 'warning').length;
    ...
    return {
      ...
      summary: {
        ...
        infos: 0,
        hints: 0,
        files_with_issues: new Set(transformedIssues.map((i: any) => i.location.file_path)).size
      },
      ai_analysis: {
        processed: false,
        summary: null,
