...
// Base Agent Class
export class BaseAgentWrapper {
    config;
    agentPath;
    constructor(agentPath, config = {}) {
...
export class CodeCompilationAgentWrapper extends BaseAgentWrapper {
    ...
    async compileProject(projectPath, projectType, buildOptions) {
        try {
...
export class CodeGenerationAgentWrapper extends BaseAgentWrapper {
    ...
    async generateProject(specification, outputPath, technology) {
        try {
    ...
    async generateCodeFromTemplate(templateType, name, options) {
        try {
...
export class CodeReviewAgentWrapper extends BaseAgentWrapper {
    ...
    async reviewProject(projectPath, reviewScope = ['quality', 'security']) {
        try {
            const scriptPath = path.join(this.agentPath, 'main_enhanced_agentic.py');
            return await this.executePythonScript(scriptPath, [{
...
export class GitHubSearchAgentWrapper extends BaseAgentWrapper {
    ...
    async searchRepositories(query, language, minStars = 0) {
        try {
...
export class APISpecWriterAgentWrapper extends BaseAgentWrapper {
    ...
    async generateAPISpec(requirements, projectName, technology) {
        try {
    ...
    async validateAPISpec(specPath) {
        try {
...
export class FixCompilationIssuesAgentWrapper extends BaseAgentWrapper {
    ...
    async fixCompilationIssues(projectPath, errors, buildTool = 'maven', buildOutput) {
        try {
...
export class AgentOrchestrator {
    ...
    async executeWorkflowPhase(phaseId, input) {
        try {
            switch (phaseId) {
                case 'specification':
                    const apiAgent = this.getAgent('apispec');
                    return await apiAgent.generateAPISpec(input.requirements, input.projectName || 'Generated Project', input.technology);
                case 'code_generation':
                    const genAgent = this.getAgent('generation');
                    return await genAgent.generateProject(input.specification, input.outputPath, input.technology);
                case 'code_review':
                    const reviewAgent = this.getAgent('review');
                    return await reviewAgent.reviewProject(input.projectPath, input.reviewScope || ['quality', 'security']);
                case 'compilation':
                    const compileAgent = this.getAgent('compilation');
                    return await compileAgent.compileProject(input.projectPath, input.projectType, input.buildOptions);
                case 'deployment':
